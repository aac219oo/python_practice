...
for i in range(2):
    while True:
        try:
            ...
            if inputYear > 0:
                yearsContainer.append(inputYear)
                break
            else:
                print("請輸入西元後")
        except ValueError:
...
for i in range(yearsContainer[0], yearsContainer[1] + 1):
        if isLeapYearFn(i)[0]:
            leapYaersContainer.append(i)
            leapYaerCount += 1
print(f"西元{yearsContainer[0]}年到西元{yearsContainer[1]}年之間，{leapYaersContainer}為閏年")
print(f"西元{yearsContainer[0]}年到西元{yearsContainer[1]}年總共有{leapYaerCount}個閏年")
